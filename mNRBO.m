function [Best_Score, Best_Pos, CG_curve,delta1] = mNRBO(N, MaxIt, LB, UB, dim, fobj)
...
sgm = 0.5;
    ...
    Position = initialization1(N, dim, UB, LB, fobj);
    ...
    for it = 1:MaxIt
        % Dynamic parameter delta, decreases over iterations
          omega=-2*pi;
          delta1(it)= 2*(1-it/MaxIt)*sin(omega *rand+pi);
        ...
        for i = 1:N                
            ...
            Xupdate = zeros(1, dim);
            for j = 1:dim                                                                       
                X3 = Position(i,j) - delta1(it) * (X2(j) - X1(j));           
                a1 = rand; a2 = rand;
            ...
            % Trap Avoidance Operator to prevent local optima
              if (q_table(1,1) <= q_table(1,2))
                ...
                if u1 < 0.5
                    X_TAO = Xupdate +  theta1 * (u1 * Best_Pos - u2 * Position(i,:)) + theta2 *delta1(it)* (u1 * mean(Position) - u2 * Position(i,:));
                else
                    X_TAO = Best_Pos + theta1 * (u1 * Best_Pos - u2 * Position(i,:)) + theta2 * delta1(it)* (u1 * mean(Position) - u2 * Position(i,:));  
                end
                Xnew = X_TAO;

             else
                    
                       alpha=rand()*((1/MaxIt^2)*it^2-2/MaxIt*it+1);
        
                        Step_length=levy(N,dim,1.5);
        
                      Xnew=Best_Pos+Step_length(i,:).*alpha.*(Best_Pos- (2*it/MaxIt)*Position(i,:));
